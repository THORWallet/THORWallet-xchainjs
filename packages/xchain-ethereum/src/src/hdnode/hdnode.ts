...
import {computeAddress as ethersComputeAddress} from '@ethersproject/transactions';
import {Wordlist, wordlists} from '@ethersproject/wordlists';
import {bip32} from '../../../xchain-crypto/src';
import {SigningKey} from '../signingkey';
import {computeAddress} from '../signingkey/address';
import {nativeSha256, pbkdf2, sha256} from './crypto';
...
export async function mnemonicToSeed(
  ...
  const buf = await pbkdf2(mnemonic, salt);
...
export async function mnemonicToEntropy(
  ...
  if (words.length % 3 !== 0) {
    throw new Error(
      'invalid mnemonic (HDNode), word length not divisable by 3',
    );
  }
  ...
  const checksum =
    new Uint8Array(await nativeSha256(entropy.slice(0, entropyBits / 8)))[0] &
    checksumMask;
...
export async function entropyToMnemonic(
  ...
  const checksum =
    arrayify(await sha256(entropy))[0] & getUpperMask(checksumBits);
...
async function base58check(data: Uint8Array): Promise<Promise<string>> {
  return Base58.encode(
    concat([data, hexDataSlice(await sha256(await sha256(data)), 0, 4)]),
  );
}
...
export class HDNode implements ExternallyOwnedAccount {
  ...
  constructor(
    ...
    defineReadOnly(this, 'fingerprint', async () =>
      hexDataSlice(ripemd160(await sha256(this.publicKey)), 0, 4),
    );
    ...
    if (mnemonicOrPath === null || mnemonicOrPath === undefined) {
  ...
  extendedKey(): Promise<string> {
    ...
    return base58check(
      concat([
        this.privateKey !== null && this.privateKey !== undefined
          ? '0x0488ADE4'
          : '0x0488B21E',
        hexlify(this.depth),
  ...
  private async _derive(index: number): Promise<HDNode> {
    ...
    const hmac: Uint8Array = new Uint8Array(
      await bip32.hmacSHA512(
        Buffer.from(arrayify(this.chainCode)),
        Buffer.from(data),
      ),
    );
  ...
  static async _fromSeed(seed: BytesLike, mnemonic: Mnemonic): Promise<HDNode> {
    ...
    const hmac: Uint8Array = new Uint8Array(
      await bip32.hmacSHA512(Buffer.from(MasterSecret), Buffer.from(seedArray)),
    );
  ...
  static async fromMnemonic(
    ...
    mnemonic = await entropyToMnemonic(
  ...
  static async fromExtendedKey(extendedKey: string): Promise<HDNode> {
    ...
    if (
      bytes.length !== 82 ||
      (await base58check(bytes.slice(0, 78))) !== extendedKey
    ) {
...
export async function isValidMnemonic(
  mnemonic: string,
  wordlist?: Wordlist,
): Promise<boolean> {
  try {
