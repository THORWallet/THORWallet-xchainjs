...
export const getAccount = async ({
  ...
  try {
    const result: AddressBalance | null = await axios
      ...
      .then((response) =>
        isErrorResponse(response.data) ? null : response.data,
      );
...
export const getTransactions = async ({
  ...
  address,
  params,
}: AddressParams & {params: TransactionsQueryParam}): Promise<
  Transaction[]
> => {
  try {
    const result: Transaction[] | null = await axios
...
export const getUnspentTransactions = async ({
  haskoinUrl,
  address,
}: AddressParams): Promise<TxUnspent[]> => {
  try {
